...
export default function OutsideWorkRequestPage() {
  ...
  const onSubmit = (data: FormData) => {
    ...
    mutation.mutate(payload, {
      onSuccess: () => {
        toast.success('ສົ່ງຄຳຮ້ອງອອກນອກສະຖານທີ່ສຳເລັດ')
        navigate('/employee/requests', { state: { tab: 'outside' } })
      },
      onError: (error) => {
  ...
  return (
    <div className="px-4 py-4">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">ໜ້າວຽກ</label>
          <textarea {...register('reason')} rows={3} placeholder="ໃສ່ໜ້າວຽກ..." className="w-full rounded-xl border border-gray-300 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#0D2B6B] resize-none" />
